...
- Custom Oretha icon family added 2026-09-14: `src/components/brand/OrethaIcon.tsx` provides inline SVG glyphs for hub, chats, office, workflows, media, guides, profile, pulse, mail, board, directory, connectors, and media modes. Hub cards, bottom nav, office tabs, media lab, and shared segmented controls use the new gold/violet ANKH linework; Lucide remains supported as a fallback. `npx tsc --noEmit` and `npm run build` pass.
